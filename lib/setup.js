...
setup.add = async function (settings) {
  await setup.checkSettings(settings)
 ...
 * @param {object} settings
 */
setup.checkSettings = async function (settings) {
  if (!settings.name) {
  ...
  const _exists = _paths.map((fs) => {
    ...
    return (async () => {
      const exists = await tools.fs.exists(settings[fs])
      if (!exists) {
        log.warn('service-systemd', `path ${settings[fs]} (${fs}) does not exists`)
      }
    })()
  })
...
setup.addScripts = async function (settings, contents) {
  ...
  await Promise.each(_tasks, (task) => {
    return task()
  })
}
