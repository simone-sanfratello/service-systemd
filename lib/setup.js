...
/**
 * exit program without error (state = 1)
...
/**
 * check if running user has root grants
 * if not, exit with error
 * @param {string} user
 * @return {bool} valid user or not
 */
setup.checkUser = function (user) {
  return user === 'root'
}

/**
 * get argument in command line
 * @params {string[]} argv should be process.argv
...
/**
 * add the service
 * exit program with success or error state
 * can also be used to update the service
 * @param {Object} settings
 * @return {Promise} Promise.resolve(message:string)
 */
...
setup.checkSettings = function (settings) {
  return new Promise((resolve, reject) => {
    ...
    // clear log and error settings on node
    if (settings.engine === 'node') {
      settings.log = settings.error = null
    }
...
setup.parse = function (settings) {
  ...
  _service = tools.string.template(_service, settings, true)

  return {
    service: _service,
    start: tools.string.template(_templates.engines[settings.engine].start, settings),
    stop: tools.string.template(_templates.engines[settings.engine].stop, settings),
    logrotate: settings.logrotate
      ? tools.string.template(_templates.logrotate, settings)
      : ''
  }
}
 ...
 * @param {Object} contents.start
 * @param {Object} contents.stop
 * @param {Object} contents.service
 * @return {Promise}
 */
setup.addScripts = function (settings, contents) {
  ...
  if (contents.start) {
    _tasks.push(() => {
      ...
      return fs.writeFileAsync(_start, contents.start, 'utf8')
    })
  }
  ...
  if (contents.stop) {
    _tasks.push(() => {
      ...
      return fs.writeFileAsync(_stop, contents.stop, 'utf8')
    })
  }
  ...
  _tasks.push(() => {
    ...
    return fs.writeFileAsync(_service, contents.service, 'utf8')
  })
...
setup.addLog = function (settings) {
  const _tasks = []
  if (settings.log) {
    let _dir = path.dirname(settings.log)
    log.info('', `ensure dir ${_dir} ...`)
    _tasks.push(fs.ensureDirAsync(_dir))
  }
  if (settings.error) {
    let _dir = path.dirname(settings.error)
    log.info('', `ensure dir ${_dir} ...`)
    _tasks.push(fs.ensureDirAsync(_dir))
  }
...
setup.addLogrotate = function (settings, contents) {
  return new Promise((resolve, reject) => {
    ...
    fs.writeFileAsync(
      path.join('/etc/logrotate.d/', settings.name),
      contents.logrotate, 'utf8')
      .then(resolve)
      .catch(reject)
  })
}
