...
setup.args = function (argv) {
  commander
    .version(app.version)
    .description('Manage systemd services for Node.JS apps. Either add, remove or print action is required. ')
    .option('-z, --print', 'Print service file, do not apply any changes')
...
setup.makeServiceFile = function (settings) {
  return setup.checkSettings(settings)
    .then(() => setup.parse(settings))
...
setup.add = function (settings) {
  ...
  // check settings data
  return setup.makeServiceFile(settings).then(contents => {
    return setup.addScripts(settings, contents).then(() => {
...
setup.checkSettings = function (settings) {
  ...
  settings['engine.bin'] = settings['engine.bin'] || execSync('which ' + settings.engine, {
    encoding: 'utf8'
  }).trim();
...
setup.parse = function (settings) {
  ...
  settings.envs = '';
  if (settings.env) {
    settings.env.forEach(value => {
      settings.envs += `Environment=${value}\n`;
    });
  }

  if(settings.envFile) {
    settings.envs += 'EnvironmentFile=' + settings.envFile;
  }
